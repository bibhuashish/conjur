...
check_if_gce_instance_is_running() {
  if [ "${INSTANCE_EXISTS}" = "1" ]; then
    local status=$(gcloud compute instances describe "${INSTANCE_NAME}" --zone=$GCE_ZONE --format="value(status)")
...
check_if_gce_instance_exists() {
  gcloud compute instances describe ${INSTANCE_NAME} --zone="${GCE_ZONE}" --format="value(name)"
  local error_level="$?"
  ...
  if [ "${error_level}" = "0" ]; then
    ...
    echo "${PROGNAME}: ${LINENO} GCE instance '${INSTANCE_NAME}' NOT found in zone: '$GCE_ZONE'!"

    # check if there are any instances in the given zone
    instances=$(gcloud compute instances list --limit=1 --format="value(name)" --zones=$GCE_ZONE)

    if [ ${#instances} -eq 0 ]; then
      error_exit "No instances found in zone: '$GCE_ZONE'"
    else
      gcloud compute instances list --zones=$GCE_ZONE \
      --format="table[box,title='Instances in zone: $GCE_ZONE'](name,machine_type.basename(),status)"
    fi
    exit 1
  fi
}
...
set_zone "$2"
echo "-- Checking if GCE instance: '${INSTANCE_NAME}' exists in zone: '$GCE_ZONE'..."
check_if_gce_instance_exists
